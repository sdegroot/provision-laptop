...
### Fixed
- **`~/.testcontainers.properties` now generated by the `containers` module**
  from the live `podman machine inspect` socket path, instead of being a
  static dotfile. Podman 5.x moved the host-side socket from
  `~/.local/share/containers/podman/machine/podman.sock` (the path the old
  dotfile referenced — gone in 5.x) to `$TMPDIR/podman/podman-machine-default-api.sock`.
  The official `podman-mac-helper install` workflow that maintains
  `/var/run/docker.sock` is broken on 5.x — its launchd plist hardcodes the
  pre-5.x path (containers/podman#18784). Generating the properties file at
  apply time sidesteps both: the path is correct on every provision, on any
  Mac/account, with no sudo or upstream tooling.
