...
### Fixed
- **Testcontainers config moved from `~/.testcontainers.properties` to shell rc.**
  Real test runs against Testcontainers 2.x revealed two failure modes the
  properties file couldn't solve:
  1. **Ryuk volume-mount fails on Podman macOS.** Testcontainers asks Podman
     to bind-mount the host socket into Ryuk as `/var/run/docker.sock`.
     Podman's applehv VM forwards `/var/folders` via virtiofsd, so the source
     path is visible inside the VM — but it's a unix socket inode, not a
     directory, and virtiofs returns `EOPNOTSUPP` when the in-VM volume setup
     tries to `mkdir` it. Fix: set `TESTCONTAINERS_DOCKER_SOCKET_OVERRIDE=/var/run/docker.sock`
     so the bind-mount happens entirely inside the VM.
  2. **`testcontainers.ryuk.disabled` properties-file key is silently ignored
     in TC 2.x** — `ResourceReaper` only checks the `TESTCONTAINERS_RYUK_DISABLED`
     env var. The properties-file key was dropped without an announced
     deprecation between 1.x and 2.x.

  The fix is to set three env vars in `.zshrc` / `.bashrc` (`DOCKER_HOST`,
  `TESTCONTAINERS_DOCKER_SOCKET_OVERRIDE`, `TESTCONTAINERS_RYUK_DISABLED`),
  with `DOCKER_HOST` discovered dynamically from `podman machine inspect`.
  This matches the official Testcontainers recommendation for rootless Podman
  on macOS (https://java.testcontainers.org/supported_docker_environment/).
  The `containers` apply step now removes any pre-existing
  `~/.testcontainers.properties`; `check`/`plan` flag it as drift if present.
...
- **Diagnostics tools** — added `smartmontools`, `perf`, `nethogs`, and `tcpdump`.
...
- **1Password SSH signing prompts on every commit during rebase** — replaced
  `op-ssh-sign` with a caching wrapper (`git-ssh-sign`) that extracts the
  signing key from 1Password once via `op read`, caches it in a dedicated
  `ssh-agent` for 1 hour, and signs subsequent commits instantly via
  `ssh-keygen`. Falls back to `op-ssh-sign` if the cache is unavailable.
  SSH authentication (push/pull) remains unchanged via 1Password agent.
...
- **monitors.xml** — add EDID-only matching policy so GNOME remembers display
  ...
  synced docked (Dell S2722DC + laptop) and undocked configurations.
