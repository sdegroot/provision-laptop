...
if [[ -z "$PROVISION_ROOT" ]]; then
    ...
    if [[ -f "$wrapper_src" ]]; then
        if diff -q "$wrapper_src" "$brave_wrapper_dest" &>/dev/null; then
            log_ok "1Password BrowserSupport wrapper deployed for Brave"
        else
            log_error "1Password BrowserSupport wrapper missing or outdated for Brave"
            drift_found=1
        fi
    fi
    ...
    if [[ -f "$brave_manifest_src" ]]; then
        ...
        if [[ -f "$brave_manifest_dest" ]] && [[ "$expanded_brave_manifest" == "$(cat "$brave_manifest_dest")" ]]; then
            log_ok "1Password native messaging manifest deployed for Brave"
        else
            log_error "1Password native messaging manifest missing or outdated for Brave"
            drift_found=1
        fi
    fi
